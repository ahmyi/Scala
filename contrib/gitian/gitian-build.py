...
gsigs = 'https://github.com/scala-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('scala'):
        subprocess.check_call(['git', 'clone', args.url, 'scala'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/scala/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'scala='+args.commit, '--url', 'scala='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/scala-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/scala/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/scala/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/scala-project/scala', help='Specify the URL of the repository. Default is %(default)s')
    ...
    args.nomac = False
    if 'm' in args.os and not os.path.isfile('builder/inputs/MacOSX10.12.sdk.tar.gz'):
        if args.build:
    ...
    os.makedirs('builder/inputs/scala', exist_ok=True)
    os.chdir('builder/inputs/scala')
    if args.pull:
